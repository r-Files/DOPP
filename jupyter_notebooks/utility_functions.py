...
def get_yearly_deaths(df: pd.DataFrame, custom_index: list = None, include_zero: bool = True) -> pd.Series:
    ...
    perennial_disasters = df.loc[
        df["Duration_CalendarYears"] != 0, ["Yearly_Disaster_Deaths", "Duration_CalendarYears"] + index_labels]
    ...
    if include_zero:
        ...
        deaths_per_year = empty_series.add(intra_year_disaster_deaths, fill_value=0).astype(
            "float32")  # will be filled with perennial disaster_deaths later
    else:
    ...
    def _flatten_death_distr(year_df: pd.DataFrame):
        ...
        yearly_deaths_by_duration: pd.DataFrame = year_df[
            ["Duration_CalendarYears", "Yearly_Disaster_Deaths"] + custom_index].groupby(
            ["Duration_CalendarYears"] + custom_index).sum()

        add_series: pd.DataFrame
        for Duration_CalendarYears, duration_df in yearly_deaths_by_duration.groupby(level="Duration_CalendarYears"):
            ...
            year_series = np.asarray(year_range).repeat(len(duration_df))

            add_series = pd.concat([duration_df] * len(year_range), ignore_index=True)
            add_series["Year"] = year_series
            add_series.set_index(index_labels, inplace=True)
            add_series = add_series["Yearly_Disaster_Deaths"]
